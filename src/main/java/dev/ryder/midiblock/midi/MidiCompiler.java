...
public final class MidiCompiler {
    ...
    private CompiledSong compile(Song song, String sourceHash) throws IOException, InvalidMidiDataException {
        ...
        long duration = toMicros(sequence.getTickLength(), sequence, tempoMap);
        Extraction extraction = extractPerformance(rawEvents, sequence, tempoMap, duration);
    ...
    private Extraction extractPerformance(List<RawEvent> events, Sequence sequence, List<TempoPoint> tempoMap, long songDurationMicros) {
        ...
        for (RawEvent event : events) {
            ...
            long timeMicros = toMicros(event.tick(), sequence, tempoMap);
            if (message.getCommand() == ShortMessage.PROGRAM_CHANGE) {
                ...
                MidiControlEvent.Type type = switch (message.getData1()) {
    ...
    private long toMicros(long targetTick, Sequence sequence, List<TempoPoint> tempos) {
        if (sequence.getDivisionType() != Sequence.PPQ) {
            return Math.round(targetTick * 1_000_000D / (sequence.getDivisionType() * sequence.getResolution()));
        }
        long micros = 0L;
        long previousTick = 0L;
        int tempo = DEFAULT_TEMPO;
        for (TempoPoint point : tempos) {
            if (point.tick() > targetTick) break;
            micros += Math.round((point.tick() - previousTick) * (double) tempo / sequence.getResolution());
            previousTick = point.tick();
            tempo = point.microsecondsPerQuarter();
        }
        return micros + Math.round((targetTick - previousTick) * (double) tempo / sequence.getResolution());
    }
