package dev.ryder.midibox.command;
...
public final class MusicCommand implements CommandExecutor, TabCompleter {
    ...
    public MusicCommand(MidiBoxPlugin plugin, SongLibrary library, PlaybackService playback, PlayerSettingsStore settings, EnhancedAudioService enhancedAudio, OrchestraProfile orchestra, JukeboxService jukeboxes, MusicMenu menu) {
        this.plugin = plugin;
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if (!(sender instanceof Player player)) {
            sender.sendMessage(ChatColor.RED + "MidiBox commands are player-only.");
            return true;
        }
        if (!player.hasPermission("midibox.use")) {
            player.sendMessage(ChatColor.RED + "You do not have permission to use MidiBox.");
            return true;
        }
        if (args.length == 0) {
            menu.open(player, 0);
    ...
    private void list(Player player) {
        if (library.songs().isEmpty()) {
            player.sendMessage(ChatColor.YELLOW + "No songs yet. Add .mid files to plugins/MidiBox/songs.");
            return;
        }
        player.sendMessage(ChatColor.GOLD + "MidiBox library (" + library.songs().size() + "):");
    ...
    private Audience resolveAudience(Player initiator, String[] args) {
        ...
        if (last.equalsIgnoreCase("global")) {
            if (!initiator.hasPermission("midibox.play.global")) return denied(initiator, "global playback");
            targets = List.copyOf(Bukkit.getOnlinePlayers());
            end--;
        } else if (args.length >= 4 && args[args.length - 2].equalsIgnoreCase("radius")) {
            if (!initiator.hasPermission("midibox.play.radius")) return denied(initiator, "radius playback");
            try {
            ...
            if (!initiator.hasPermission("midibox.play.others")) return denied(initiator, "player-targeted playback");
    ...
    private void requestPack(Player player) {
        if (enhancedAudio.request(player)) player.sendMessage(ChatColor.AQUA + "Requested the MidiBox enhanced audio pack.");
    ...
    private void orchestra(Player player, String[] args) {
        if (!player.hasPermission("midibox.admin")) {
    ...
    private void analyze(Player player, String[] args) {
        ...
        library.find(songId).ifPresentOrElse(song -> playback.diagnostics(song).ifPresentOrElse(diagnostics -> {
            player.sendMessage(ChatColor.GOLD + "MidiBox analysis: " + ChatColor.WHITE + song.displayName());
    ...
    private void playlist(Player player, String[] args) {
        if (args.length < 2 || args[1].equalsIgnoreCase("list")) {
            List<dev.ryder.midibox.profile.Playlist> playlists = settings.playlists(player);
    ...
    private void playPlaylist(Player player, dev.ryder.midibox.profile.Playlist playlist) {
    ...
    private void reload(Player player) {
        if (!player.hasPermission("midibox.admin")) {
            player.sendMessage(ChatColor.RED + "You do not have permission to reload MidiBox.");
            return;
        }
