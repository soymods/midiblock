package dev.ryder.midibox.playback;

import dev.ryder.midibox.library.Song;
import dev.ryder.midibox.arrangement.ArrangementCompiler;
import dev.ryder.midibox.arrangement.ArrangementDiagnostics;
import dev.ryder.midibox.arrangement.NoteBlockArrangement;
...
import dev.ryder.midibox.midi.MidiControlEvent;
...
public final class PlaybackService {
    ...
    public StartResult play(Player player, Song song, float volume) {
        ...
        return requestPlay(player, song, volume, 0L);
    }

    /** Starts an already-running shared source at its current song position. */
    public StartResult playAt(Player player, Song song, float volume, long positionMicros) {
        queues.remove(player.getUniqueId());
        return requestPlay(player, song, volume, Math.max(0L, positionMicros));
    }
    ...
    public void skip(Player player) {
        ...
        stop(player);
        if (next != null) requestPlay(player, next, volume, 0L);
    }

    private StartResult requestPlay(Player player, Song song, float volume, long positionMicros) {
        stop(player);
        ...
        return compiler.ready(song).flatMap(compiled -> arrangementCompiler.ready(song, compiled)).map(arrangement -> {
            start(player, song, arrangement, volume, requestId, positionMicros);
            return StartResult.STARTED;
        }).orElseGet(() -> {
            CompletableFuture.supplyAsync(() -> prepare(song), compilerExecutor).whenComplete((prepared, error) ->
                Bukkit.getScheduler().runTask(plugin, () -> completeCompilation(player.getUniqueId(), song, volume, positionMicros, requestId, prepared, error))
            );
    ...
    public Optional<Long> durationMicros(Song song) {
        return compiler.ready(song).map(CompiledSong::durationMicros);
    }
    ...
    private void completeCompilation(UUID playerId, Song song, float volume, long positionMicros, long requestId, PreparedSong prepared, Throwable error) {
        if (!Long.valueOf(requestId).equals(requests.get(playerId))) return;
        Player player = Bukkit.getPlayer(playerId);
        if (player == null || !player.isOnline()) return;
        if (error != null) {
            requests.remove(playerId);
            player.sendMessage("§cMidiBox could not compile " + song.displayName() + ". Check the server log.");
            plugin.getLogger().warning("Could not compile " + song.id() + ": " + error.getMessage());
            return;
        }
        start(player, song, prepared.arrangement, volume, requestId, positionMicros);
